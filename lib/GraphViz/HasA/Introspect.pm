...
sub find_attributes_in_class {
    my ($self, $class) = @_;
    return sort { $a->name cmp $b->name } $class->get_all_attributes;
}

sub find_links_from_class {
    my ($self, $class) = @_;

    return map { $self->find_links_from_attribute($class, $_) }
        $self->find_attributes_in_class($class);
}

sub find_attributes_in_role {
    my ($self, $role) = @_;
    return sort { $a cmp $b }
        uniq map { $_->get_attribute_list } $role->calculate_all_roles;
}

sub find_links_from_role_attribute {
    my ($self, $role, $name) = @_;
    ...
    return map { [ $name => { %result, to => $_ } ] } @classnames;
}

sub find_links_from_role {
    my ($self, $role) = @_;
    return map { $self->find_links_from_role_attribute($role, $_) }
        $self->find_attributes_in_role($role);
}
